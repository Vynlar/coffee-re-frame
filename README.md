...
* [`src/clj/coffee_re_frame/`](src/clj/coffee_re_frame/): Backend and middleware source files (Clojure,
[Compojure](https://github.com/weavejester/compojure))
* [`src/cljs/coffee_re_frame/`](src/cljs/coffee_re_frame/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/coffee_re_frame/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/coffee_re_frame/`](test/cljs/coffee_re_frame/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `coffee-re-frame` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/coffee_re_frame/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns coffee-re-frame.example
  (:require [coffee-re-frame.config :as config])
...
standalone `coffee-re-frame.jar`.
...
```sh
port=2000 java -jar target/coffee-re-frame.jar
```
